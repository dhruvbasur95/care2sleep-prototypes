 ...
 * withdraw, reassign). Copy convention (not shipped in this package, recorded
 * here so the rule survives): the title
...
export function ConfirmDialog({
  ...
  /* ── There is no `variant` prop, and that is deliberate ──────────────────
     In the full Care2Sleep prototype this chassis is shared with the Research
     Dashboard and the Coach Delivery Portal, and carried a
     `variant: 'app' | 'consumer'` flag: a 440px panel with 14px buttons for
     those two, a 640px panel with 48px controls for this one. This package
     ships the Consumer Portal alone, all five call sites passed
     `variant="consumer"`, and the 'app' branch was the only thing still
     painting the coach/researcher purple (`--primary` #4a278f) inside a
     consumer surface. It is removed rather than left as dead configuration.

     The sizing below is therefore not arbitrary — it is this portal's
     accessibility floor, from a direct instruction: "the pop-up modal is too
     small for someone with poor digital literacy". Width is
     `min(92vw, 640px)`, so the panel is capped by the viewport rather than a
     breakpoint and cannot overflow a 375px phone; padding steps 24 -> 32 at
     `md`; every control is 48px. If you reintroduce a compact variant for
     another audience, do it as a prop again — never by shrinking these. */
  /**
  ...
  const trapKeys = (e: React.KeyboardEvent) => {
    ...
    /* ⚠️ `textarea` belongs in this list and was missing until it was measured.
       `ConsumerAccountPage`'s opt-out dialog renders a real <textarea> inside
       this chassis; without it here, `first`/`last` were computed from the
       buttons alone, so Tab reached the message box exactly once and then
       wrapped permanently past it. Keep this selector in step with every
       natively focusable element the dialog can contain. */
    const focusables = [
          ...
          <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center p-6">
            <motion.div
              ...
              className={cn(
                ...
                panelClassName ?? 'max-h-[90vh] w-[min(92vw,640px)] p-6 md:p-8',
              )}
            >
              {!hideHeader && (
              <>
                  ...
                  <>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        /* ⚠️ The FILL is variant-aware, and has to be. Every
                           other colour on this button was already gated on
                           `consumer` while the fill was not, so a consumer
                           dialog painted Cancel's border in `consumer-primary`
                           #3a00ad and Continue's fill in the coach/researcher
                           `--primary` #4a278f — two purples 9px apart in one
                           footer. The consumer portal's own filled-CTA
                           convention is `hover:opacity-90`; it has no `-hover`
                           shade token, so do not reach for one.
                           `destructive` is deliberately NOT branched: red
                           carries the same meaning in both portals and both
                           read it from the one semantic token. */
                        confirmDisabled
                          ? 'cursor-not-allowed bg-consumer-primary/40'
                          : destructive
                            ? 'bg-destructive hover:bg-destructive/90'
                            : 'bg-consumer-primary hover:opacity-90',
                      )}
                    >
                      {confirmLabel}
                    </button>
