...
function DiaryMascot({ className }: { className?: string }) {
  ...
  /* ⚠️ The pillow's breath is started IMPERATIVELY, and it has to be.
     Do not "simplify" it back to `animate={{ scale: [...] }}`.

     `AnimatePresence initial={false}` writes `initial: false` onto
     `PresenceContext`, and that context reaches EVERY descendant — not only the
     direct presence children. A `motion` component mounting under it is told
     "no entrance", so it applies its `animate` target with no transition, which
     for a keyframe ARRAY means snapping to the final keyframe and never
     looping. An explicit `initial` prop does NOT override this: the presence
     check runs either way.

     `ConsumerDiaryPage`'s stage switcher carries `initial={false}` on purpose,
     so stages do not animate on arrival. The cost was that this mascot sat
     perfectly still on the diary's WELCOME screen while the identical component
     breathed correctly on the THANK-YOU screen — that one mounts later, once
     presence is no longer "initial". Measured before the fix: `transform: none`
     from the first frame, zero style writes in 3s under a MutationObserver.

     An effect runs a commit AFTER mount, by which time presence is no longer
     initial, so `controls.start()` animates normally wherever it is mounted.
     Any infinite keyframe animation inside an `initial={false}` subtree needs
     this same treatment. */
    ...
    <div className={cn('relative aspect-[263/145.5]', className)} aria-hidden="true">
      ...
      */}
      <motion.div
        ...
        /* See the note at the top of `DiaryMascot`: the breath is driven by
           `breath.start()` in an effect, not by an `animate` prop. */
        animate={breath}
      >
        <motion.div
