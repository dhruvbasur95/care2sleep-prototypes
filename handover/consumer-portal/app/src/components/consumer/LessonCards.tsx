...
export function LessonPlayCta({
  ...
  /* ⚠️ No destination yet — so this renders as a FOCUSABLE, `aria-disabled`
     button carrying an `sr-only` reason, never as a silently dead control.
     That is this project's standing rule for anything designed but not yet
     wired, and this was the package's only violation of it: three "Play
     module" buttons on My Modules were styled, 48px, carried a real
     `aria-label`, and did absolutely nothing when clicked — indistinguishable
     from a broken app.

     `aria-disabled` rather than `disabled`, deliberately: a `disabled` button
     is removed from the tab order, so a screen-reader user never reaches it
     and never learns the module exists. This way they reach it, hear the
     label, and hear why it will not open.

     Six of the seven modules have no inner pages built (`consumerLessonContent`
     holds one), which is what `to` being undefined means. When content lands,
     pass `to` and this branch stops being reached. */
  return (
    <button
      type="button"
      aria-disabled="true"
