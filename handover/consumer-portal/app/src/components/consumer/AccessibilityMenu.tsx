  ...
  return (
    <button
      ...
      className={cn(
        ...
        /* 55%: `ink` on `parchment` at 55% opacity rasterises to rgb(125,125,126)
           against the tile — **3.78:1**.

           ⚠️ This comment previously claimed "4.61:1 and still clears AA". Both
           halves were wrong, and re-measuring through a 1x1 canvas is what
           caught it. 3.78:1 does NOT clear AA for normal text. It is shipped
           anyway because this styling applies ONLY to a control carrying
           `aria-disabled` (see the prop above), and WCAG 1.4.3 exempts
           inactive controls — so the honest statement is "exempt", not "passes".
           If this tint is ever reused on an ENABLED control it has to darken
           first.

           The stacking note below is still true and still the reason not to
           reach for the obvious fix: `opacity-50` on the whole tile takes the
           border down with it and lands the label near 3.9:1 on a control that
           is not exempt — the same mistake Round 14.5 fixed on `WeekdayPicker`. */
