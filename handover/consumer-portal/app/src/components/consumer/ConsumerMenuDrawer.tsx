    ...
    <AnimatePresence>
      {open && dyadId && (
        <motion.div
          ...
          /* ⚠️ The accessibility text scale has to be applied HERE, not
             inherited. `ConsumerShell` puts `zoom` on `<main>` only — correctly,
             so the header holding the menu does not grow under the finger
             pressing it — but this drawer is mounted in `App.tsx` ABOVE the
             routes (see the note there), so it sits outside `<main>` and
             inherited nothing. Below 1200px this drawer IS the navigation, so a
             reader who scaled the page up to read it was then handed unscaled
             nav to get anywhere. Any other surface mounted outside `<main>`
             needs this same line. */
          style={{ zoom: scale }}
