...
import { releasedLessons } from './lessons'
 ...
 *    reproduce a card that contradicts itself, both numbers come from one
 *    source.
 *
 *    ⚠️ **Superseded in Round 46.** That source used to be a hardcoded
 *    `DEMO_PROGRESS = 143/264` transcribed from the frame, and this note used to
 *    say the card reads "~7 mins left". Wiring the card to the real module
 *    player removed it: the bar and the caption now both come from
 *    `releasedLessons`, so the card shows this dyad's actual progress and the
 *    figure moves with the data. The frame's own 54% is no longer reproduced at
 *    all, and that is the right trade — a real number beats a faithful one.
 *
...
/*
 * ⚠️ Round 46 removed this card's demo state cycler (direct instruction: "make
 * sure the module on home page is also wired").
 *
 * The CTA used to be a button that walked start -> resume -> complete on every
 * click, purely so a reviewer could see all three states, and the resume bar
 * was pinned to a hardcoded `DEMO_PROGRESS = 143/264` from frame `787:1196`.
 * Both are gone: the state and the progress now come from `releasedLessons`,
 * the same helper My Modules reads, and the CTA is a real link into the module.
 *
 * What that costs, stated so it is not rediscovered as a regression: Home now
 * shows only whatever state the demo dyad's Module 4 is genuinely in. The other
 * two states are still reachable — My Modules renders every released module in
 * its own real state — so nothing became unreviewable, it just stopped being
 * reviewable from this one card.
 */
...
export function LearningTaskCard({ dyad }: { dyad: ConsumerDyad }) {
  ...
  const minutesLeft = view.minutesLeft
  // `undefined` for a module with no content built yet, which leaves the CTA a
  // plain button rather than a link into an empty player. Only Module 4 is
  // populated today.
  const href = moduleLesson(view.id) ? `/consumer/${dyad.id}/module/${view.id}` : undefined
      ...
      */}
      ...
      <div className="flex flex-1 flex-col gap-6 px-4 pt-6 pb-6 sm:gap-10 sm:px-8 sm:pt-8 sm:pb-10">
        <div className="flex flex-col gap-8">
          ...
          */}
          <div className={cn('flex flex-col', state === 'resume' ? 'gap-4' : 'gap-2')}>
            ...
            {state === 'start' && (
              <p className="text-body text-parchment">~{totalMinutes} mins to complete</p>
            )}
        ...
        {/* The `role="status"` live region that used to sit here went with the
            demo cycler: nothing on this card changes after mount any more, and
            a live region with static content is noise. The same facts are in
            the visible copy above. */}
      </div>
    </div>
  )
}
