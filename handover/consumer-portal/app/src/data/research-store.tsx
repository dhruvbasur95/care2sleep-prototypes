  ...
  const updateSessionPlanRow = useCallback(
    (dyadId: string, session: number, patch: Partial<SessionPlanRow>) => {
      setSessionPlans((prev) => {
        ...
        // Keep the date this row is moving *off*, so the consumer's plan strip
        // can render "was Wed 22 Oct" rather than an unexplained "Rescheduled".
        // Only when the date actually changes: a row edited for time alone is
        // still flagged `rescheduled` by the line below, and stamping
        // `previousDate` there would strike through a date that never moved.
        const moved = alreadySet && !!patch.date && patch.date !== row?.date
        const previous = moved ? { previousDate: row?.date } : {}
        return {
          ...prev,
          [dyadId]: {
            ...plan,
            sessions: plan.sessions.map((s) =>
              s.session === session
                ? {
                    ...
                    ...creds,
                    ...previous,
                    rescheduled: alreadySet ? true : s.rescheduled,
                  }
                : s,
            ),
          },
