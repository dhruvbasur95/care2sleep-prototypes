...
export function DeliveryShell({
  ...
  // 🚫 DO NOT ADD A SKIP BUTTON HERE. See docs/orchestration-flows.md §3.3.
  //
  // The trainee welcome flow is MANDATORY by study requirement: every trainee
  // must see the Care2Sleep induction. `DeliveryOnboarding` therefore renders
  // only "Go back" and "Go next"/"Go to my dashboard", and there is deliberately
  // no path into the portal that skips it.
  //
  // The optional `to` argument below is RETAINED ORPHANED CAPABILITY from
  // Round 32, when a Skip button did exist. It is not a hook awaiting
  // reconnection. Every live caller passes `to`, so the no-argument branch is
  // currently unreachable — that is intended, not a regression.
  //
  // Consequence, also intended: because the final CTA is what starts the tour,
  // a trainee always completes the welcome and always enters the seven-step
  // tour on first run.
  const completeOnboarding = useCallback(
    (to?: string) => {
      ...
      setOnboarded(true)
      if (to) navigate(to)
      // Round 32: the first-run tour starts here, and only from a CTA that
      // names its destination. Deferred past the crossfade so the tour measures
      // the dashboard's real boxes rather than the ones it has mid-fade.
      if (to) window.setTimeout(startDeliveryTour, TOUR_START_DELAY_MS)
    },
    [navigate],
  )

  // Finishing the welcome unmounts the control that was clicked, so focus would
  // otherwise fall to `<body>` — this project's most-repeated defect.
  // `main` carries `tabIndex={-1}` as the deliberate landing spot.
  //
  // (This comment previously named "Skip" and a "Get started" button. Neither
  // exists: Skip was removed in Round 34 and the label is "Go to my dashboard".
  // Corrected for the handover — the focus behaviour it describes is real and
  // still required.)
  //
