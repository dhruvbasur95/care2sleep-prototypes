...
function ProfileDetailsCard({ coach }: { coach: ReturnType<typeof useResearch>['coaches'][number] }) {
  ...
  /* HANDOVER PACKAGE ONLY — accessibility fix, WCAG 2.4.3.
     Measured: clicking "Edit details" dropped `document.activeElement` to
     `<body>`, and so did Save and Cancel. All three controls unmount the moment
     they are activated — "Edit details" is behind `!editing`, Save/Cancel are
     inside the form — which is this project's most-repeated defect class.
     Focus is now moved deliberately in both directions: into the first editable
     field on entering edit mode, and back onto "Edit details" on leaving it.
     `wasEditing` stops the return branch firing on first mount, when nothing
     has been left yet. */
