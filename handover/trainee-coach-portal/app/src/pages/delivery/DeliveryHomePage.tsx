 ...
 *
 * **Both CTAs are wired.** "Take tour" opens `COACH_TOUR_STEPS`; "Skip tour"
 * hides the banner.
 *
 * (This comment previously said the coach tour "does not exist yet" and that
 * "Take tour" was `aria-disabled`. Both statements were made obsolete by Round
 * 40 and were corrected for the handover — an engineer reading the stale
 * version would conclude there is no coach tour to build against. There is; see
 * `COACH_TOUR_STEPS` in `data/deliveryTour.ts`.)
 *
 * The banner is not persisted, matching the trainee onboarding and tour it sits
 * alongside, and it re-arms whenever the coach stage is entered — see the
 * handover note on `showWelcome` in `DeliveryHomePage`.
 */
...
export function DeliveryHomePage() {
  ...
  /* HANDOVER PACKAGE ONLY — not in the source prototype. Direct instruction:
     "when I select coach, by default start from tour banner state."

     Two things made that untrue, and both are demo-review problems rather than
     product ones:

     1. `showWelcome` is a single piece of state shared by both stages, in a
        component that stays mounted across a stage switch. The effect above
        hides the banner whenever *a* tour ends — so finishing or skipping the
        **trainee** tour also hid the **coach** banner, before a reviewer had
        ever seen it. That is why the banner appeared to be missing at random.

     2. Even without that, dismissing the coach banner once left no way back to
        it short of a full page reload.

     Re-arming on entry to the coach stage fixes both: selecting "Coach" always
     lands on the tour-banner state, which is the use case this package exists
     to demonstrate. `tourWasStarted` is cleared too, so the hide-on-tour-end
     effect cannot fire against a tour that belonged to the other stage.

     This is REVIEW SCAFFOLDING. In production the banner is a real first-run
     state driven by a per-coach server flag (see docs/orchestration-flows.md
     §1), not something that re-arms every time a view is opened. Delete this
     effect when that flag exists. */
  useEffect(() => {
    if (stage === 'coach') {
      tourWasStarted.current = false
      setShowWelcome(true)
