      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <button
          ...
          onClick={() => {
            ...
            // Guard replaces what the native `disabled` attribute enforced.
            if (state === 'playing') return
            play()
          }}
          aria-label={state === 'watched' ? `Watched: ${label}` : `Play: ${label}`}
          /* HANDOVER PACKAGE ONLY — accessibility fix.
             This was `disabled={state === 'playing'}`. Pressing play disabled
             the button under the user's own focus, and the browser drops focus
             from a disabled element, so `document.activeElement` fell to
             `<body>` for the whole simulated playback — measured. `aria-disabled`
             keeps it focusable while conveying the same state. */
