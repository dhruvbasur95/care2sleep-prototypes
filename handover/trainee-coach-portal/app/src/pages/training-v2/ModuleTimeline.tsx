 ...
 *
 * This ~1,700-line component was the module curriculum list. Its last caller
 * was My Learning (`/delivery/learning`), which was rebuilt in Round 30 onto
 * the Part A / Part B tier carousels it renders today. Nothing imports this
 * file: `grep -rn "ModuleTimeline" src` returns only this file itself.
 *
 * It is kept because it is heavily-iterated product logic — the lock/unlock
 * visual language, the current-module auto-centring and the `autoCenterCurrent`
 * escape hatch added for a single-scroll-container page were each the result of
 * a real defect — and because a curriculum list may return in another form.
 *
 * If you are rebuilding: read it for its rules, do not wire it back up.
 * Deleting it is a safe change.
 */
import {
  useCallback,
  useEffect,
