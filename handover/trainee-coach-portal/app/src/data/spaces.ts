...
/** Builds a health log ending on `format.ts`'s TODAY ('2026-07-22'), one day
 *  per entry in `diaryEntries` (5 days by default; a longer `diaryEntries`
 *  array — e.g. dyad-011's, extended in Round for the Sleep diary notes
 *  card's horizontal-scroll demo — simply reaches further back). */
function healthLog(
  startRem: number,
...
export const consumerDyads: ConsumerDyad[] = [
  ...
  },
  {
    ...
    moduleEngagement: consumerModuleEngagement([
      ...
      // Missed: its session (Session 3) was held without it.
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    // TODAY, 2026-07-22) to demonstrate the Data Health Alerts banner on an
    // unassigned consumer.
    patientLog: [
