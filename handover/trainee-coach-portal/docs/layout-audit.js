 ...
 *
 *    FOURTH CALIBRATION (trainee/coach handover) — My Notes (`/delivery/notes`)
 *    reports three `ViewDownload` cells. Verified false positive, same class:
 *    the "View" and "Download" links both measure `top: 963` — one line, side by
 *    side at left 522 and 570 — and the second distinct rect top comes from the
 *    36px icon button sitting beside them at `top: 954`. The cell is 210px wide
 *    holding 32 + 65 + 36px of content; nothing wraps. Left reporting rather
 *    than excluding multi-control cells, which is where the real squeeze bug
 *    would show up. */
  ...
  root.querySelectorAll('*').forEach((el) => {
    ...
    if (r.width > 0 && r.right > vw + 1 && getComputedStyle(el).position !== 'fixed') {
      const scroller = el.closest('[class*="overflow-x-auto"],[class*="overflow-auto"],[class*="overflow-hidden"]')
